    ...
    case JQueueIdx:
        {
            ...
            jobPtr = NewJob((itPtr->servPtr ? itPtr->servPtr->server : NULL),
                            queuePtr->name, job_type, objv[argIndex++]);
 ...
 *
 *      Note: the "queuelock" should be held locked.
 *
 * Results:
 *      The job.
 *
...
GetNextJob(void)
{
    Queue         *queuePtr;
    Tcl_HashEntry *jPtr;
    Job           *prev = NULL, *tmp = NULL, *jobPtr = NULL;
    int            done = 0;
