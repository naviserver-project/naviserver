...
NsConfigTcl(void)
{
    concurrent_interp_create = Ns_ConfigBool(NS_GLOBAL_CONFIG_PARAMETERS, "concurrentinterpcreate",
#ifdef NS_TCL_PRE86
                                             NS_FALSE
#else
                                             NS_TRUE
    ...
    maxConcurrentUpdates = Ns_ConfigIntRange(NS_GLOBAL_CONFIG_PARAMETERS, "maxconcurrentupdates", 1000, 1, INT_MAX);
}
...
ConfigServerTcl(const char *server)
{
    ...
    if (unlikely(servPtr == NULL)) {
        ...
        if (Ns_PathIsAbsolute(servPtr->tcl.library) == NS_FALSE) {
            Ns_HomePath(&ds, servPtr->tcl.library, NS_SENTINEL);
            n = ds.length;
 ...
 * ICtlAddTrace
 *
 *      Helper function for various trace commands
 *
 * Results:
 *      Standard Tcl result.
 *
...
ICtlSaveObjCmd(ClientData clientData, Tcl_Interp *interp, TCL_SIZE_T objc, Tcl_Obj *const* objv)
{
    ...
    if (Ns_ParseObjv(NULL, args, interp, 2, objc, objv) != NS_OK) {
        ...
        if (++servPtr->tcl.epoch == 0) {
            /*
             * Epoch zero is reserved for new interps.
             */
            ...
            while (tracePtr != NULL) {
                if (tracePtr->when == why) {
                    ...
                    if ((*tracePtr->proc)(itPtr->interp, tracePtr->arg) != TCL_OK) {
                        LogErrorInTrace(itPtr, "trace", why);
                    }
                }
...
static void
LogErrorInTrace(const NsInterp *itPtr, const char *context, Ns_TclTraceType why) {
    Tcl_DString ds;
    ...
    (void) Ns_TclLogErrorInfo(itPtr->interp, ds.string);
    Tcl_DStringFree(&ds);
}
